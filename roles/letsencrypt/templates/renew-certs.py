...
failed = False
site_hosts_hashes = {{ site_hosts_hashes }}

for site in {{ sites_using_letsencrypt }}:
    cert_path = os.path.join('{{ letsencrypt_certs_dir }}', site + '-' + site_hosts_hashes[site] + '.cert')
    bundled_cert_path = os.path.join('{{ letsencrypt_certs_dir }}', site + '-' + site_hosts_hashes[site] + '-bundled.cert')
    ...
    cmd = ('/usr/bin/env python {{ acme_tiny_software_directory }}/acme_tiny.py '
           ...
           ).format(site, site_hosts_hashes[site])
